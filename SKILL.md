...
isolated via worktrees up to the ≤2 same-target sub-cap, ≤1 kernel-extend,
...
RedBaron they run locally, so honor the ≤2 same-target sub-cap strictly. The real OOM guard is the **≤2 same-target
...
1. **Shared `build_into` → isolate with worktrees, don't serialize.**
   ...
   fastembed) are memory-hungry; 2 bounds the blast radius (2026-09-09: RedBaron OOMed at 3 with a gate overlapping, load 21k; operator set 2 until PRD-build-cargo-concurrency-budget ships: RedBaron 30 GB, worktree targets on /mnt/data). Kernel-extend
