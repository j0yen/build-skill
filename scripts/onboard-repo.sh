...
# risk-gate wiring (PRD-build-gate-producers P1): scripts/audit.sh and its
# rules/audit-checks.sh vendored fallback, at the REPO ROOT (not agent/,
# and not the resolved Cargo project root when the two differ — matches
# extend-gate.sh's own `$repo/scripts/audit.sh` check). Generated
# only-where-absent, same convention as AGENT_FILES. mcphost was hand-built
# with this wiring already pointed at the live (renamed) skill path; this
# is that same template, so a newly onboarded crate never inherits the
# stale ~/.claude/skills/autobuilder/... path that broke wm-node/adopt/
# rustbuild's risk-gate receipt for weeks after the autobuilder->rustbuild
# skill rename.
RISK_GATE_SKILL_RULES="$HOME/.claude/skills/rustbuild/rules/audit-checks.sh"
...
if [ -f "$repo/scripts/audit.sh" ]; then
  plan_audit_sh="kept"
else
  plan_audit_sh="create"
fi
if [ -f "$repo/rules/audit-checks.sh" ]; then
  plan_audit_rules="kept"
elif [ -x "$RISK_GATE_SKILL_RULES" ]; then
  plan_audit_rules="create"
else
  plan_audit_rules="skip (no canonical audit-checks.sh at $RISK_GATE_SKILL_RULES to vendor from)"
fi
...
if $check_mode; then
  ...
  echo "  scripts/audit.sh: $plan_audit_sh"
  echo "  rules/audit-checks.sh: $plan_audit_rules"
  echo "  tag: $tag_action"
...
# --- scripts/audit.sh + rules/audit-checks.sh (PRD-build-gate-producers P1) --
# Repo-root, only-where-absent, same template mcphost/wm-node/adopt/rustbuild
# now all share (PRD-build-gate-producers). Points at the LIVE (renamed)
# skill path from the start, with the same CI-safe vendored-copy fallback.
if [ "$plan_audit_sh" = "create" ]; then
  mkdir -p "$repo/scripts"
  cat > "$repo/scripts/audit.sh" <<'AUDITEOF'
#!/usr/bin/env bash
...
# ~/.claude — mirrors mcphost's scripts/audit.sh convention). Generated by
# scripts/onboard-repo.sh (PRD-build-gate-producers) — if the rustbuild
# skill is ever renamed again, update this path AND the vendored copy at
# rules/audit-checks.sh in every repo that carries one.
...
AUDITEOF
  ...
  created+=("scripts/audit.sh")
else
  kept+=("scripts/audit.sh")
fi

case "$plan_audit_rules" in
  create)
    mkdir -p "$repo/rules"
    cp "$RISK_GATE_SKILL_RULES" "$repo/rules/audit-checks.sh"
    chmod +x "$repo/rules/audit-checks.sh"
    src_sha="$(git -C "$HOME/wintermute/rustbuild" rev-parse --short HEAD 2>/dev/null || echo unknown)"
    sed -i "1a # VENDORED COPY — canonical source: j0yen/rustbuild skill/rules/audit-checks.sh @ ${src_sha}\\n# Refreshed by copying that file here; CI has no ~/.claude, so the crate must carry it." "$repo/rules/audit-checks.sh"
    created+=("rules/audit-checks.sh")
    ;;
  kept) kept+=("rules/audit-checks.sh") ;;
  *) echo "onboard-repo: skipping rules/audit-checks.sh vendoring — $plan_audit_rules" >&2 ;;
esac
...
echo
echo "  should now be unblocked (scaffolding-dependent receipts): intake, vti-plan, rollback-plan (has a real tag base), reviewer-agent, ci-checks, session-trace, proof-lane routing, risk-gate (scripts/audit.sh now points at a real audit-checks.sh)"
