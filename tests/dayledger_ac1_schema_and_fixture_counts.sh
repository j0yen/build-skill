...
# that validates against build.day_ledger.v2 (every key present, types
# correct — schema bumped to v2 by PRD-build-flow-ledger req 4, which added
# the flow{} block; v1 readers are unaffected, they just don't see flow{})
# and whose ticks/gates/shipped/landings/decisions counts match the
# fixture exactly.
set -uo pipefail
...
run_suite_and_expect_labels \
  ...
  "ok  AC1: schema (build.day_ledger.v2)" \
