#!/usr/bin/env bash
# repohealth_ac2_report_mode_is_readonly.sh — PRD-build-repo-health-invariants AC2.
...
out="$(bash "$HERE/../scripts/repo-health-selftest.sh" ac2 2>&1)"; rc=$?
echo "$out"
[ "$rc" -eq 0 ] || { echo "FAIL: repo-health-selftest.sh ac2 exited $rc" >&2; exit 1; }
