#!/usr/bin/env bash
# archverify-ac-common.sh — shared helper for the tests/archverify_ac<N>_*.sh
# per-AC wrapper files (PRD-build-archive-verify-before-shipped, test_prefix
# `archverify`). scripts/archive-commit-selftest.sh already exercises this
# PRD's postcondition/ordering ACs as a set of named `ARCHVERIFY AC<N>: ...`
# assertions against the real archive-commit.sh code, run against a
# disposable git fixture — there is no separate, hand-duplicated
# implementation per AC here, deliberately, mirroring
# tests/fixtures/manbackfill-ac-common.sh's convention exactly (a second,
# independent per-AC test body would drift from the real one and prove
# nothing an edit to the actual script couldn't silently invalidate).
