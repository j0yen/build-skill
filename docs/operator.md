...
## Full tick procedure (detail, moved wholesale from SKILL.md, PRD-build-branch-contract-split)

SKILL.md's index above summarizes each phase in one line, with links here
and to docs/branch-contract.md. This section is the coordinator's full
phase-by-phase procedure — the same text SKILL.md carried before this PRD,
moved here verbatim (headers demoted one level) so it stops being read on
every tick while staying exactly as findable. The one exception: the
"Each agent prompt must include, self-contained" dispatch-prompt-composition
block was NOT moved here — it is superseded by docs/branch-contract.md
(requirement 1), which is now the sole source for that prompt.
